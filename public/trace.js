...
//////////////////////得到手机的设备名称
    ...
    utm_source      = params.utm_source ? params.utm_source : utm_source
    utm_medium      = params.utm_medium ? params.utm_medium : utm_medium
    ...
    if(fid || ((!cookie_fid) && (utm_source || utm_medium || utm_campaign || utm_content || utm_term))){ //存在 fid，不管 utm 的各个参数是否存在，强制覆盖cookie
        ...
        if(utm_term){
            ...
            // 如果上面没有值，则这里删除掉该cookie
            Delete_Cookie( "utm_term", "/", '');
        }

    }else{
        var cookie_utm_source = Get_Cookie("utm_source");
        ...
        var cookie_utm_term = Get_Cookie("utm_term");
        if(cookie_utm_term){
            params.utm_term = cookie_utm_term;
    ...
    if(document) {
		...
		//session 保留30天
		if(cookie_sid = Get_Cookie('sid') ){
            params.sid = cookie_sid;
        }else{
            params.sid = uuid;
